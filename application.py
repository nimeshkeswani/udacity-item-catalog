...
def add_item():
    ...
    if request.method == 'POST':
        ...
        if item_name is None or item_name == '' \
                or category_id is None or category_id == '':
            flash("Empty item name or category name or both.")
            return redirect(url_for('add_item'))
        if session.query(Item).filter_by(item_name=item_name).filter_by(
...
def delete_item(category_id, item_id):
    ...
    if request.method == 'POST':
        session.delete(item)
        session.commit()
        flash("Item %s has been delted from category %s." % (item_name,
                                                             category_name))
        return redirect(url_for('items'))
    else:
...
def categories_json_post():
    ...
    if session.query(Category).filter_by(category_name=category_name)\
            .first() is not None:
...
def category_json_modify(category_id):
    ...
    if request.method == 'DELETE':
        session.delete(category)
        session.commit()
        return jsonify({'message': 'Category %s has been deleted'
...
def items_json_post():
    ...
    if item_name is None or item_name == '' \
            or category_id is None or category_id == '':
        return jsonify({'error': 'Empty item name or '
                                 'category name or both.'})
    if session.query(Item).filter_by(item_name=item_name)\
            .filter_by(category_id=category_id).first() is not None:
...
@app.route('/categories/<int:category_id>/items/<int:item_id>/api/json',
           methods=['GET'])
def item_json_get(category_id, item_id):
    item = session.query(Item)\
        ...
        .filter_by(item_id=item_id).first()
    if item is None:
        return jsonify({'error': 'No Item with id %s and '
                                 'Category id %s.' % (item_id, category_id)})
...
def item_json_modify(category_id, item_id):
    ...
    if request.method == 'DELETE':
        session.delete(item)
        session.commit()
        return jsonify({'message': 'Item %s in Category %s has '
                                   'been deleted' % (item.item_name,
                                                     item.category_id)})
